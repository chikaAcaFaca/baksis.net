    ...
    <div className="min-h-screen flex flex-col bg-[#fcfdfe]">
      ...
      <header className="fixed top-0 left-0 right-0 z-[100] bg-white/90 backdrop-blur-xl border-b border-gray-100 h-16 flex items-center shadow-sm">
        <div className="max-w-6xl mx-auto px-6 w-full flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8">
            <NavLink to="/search" className={({ isActive }: any) => `text-[10px] font-black uppercase tracking-widest ${isActive ? 'text-indigo-600' : 'text-gray-400 hover:text-gray-900'}`}>Istraži</NavLink>
            <NavLink to="/studio" className={({ isActive }: any) => `text-[10px] font-black uppercase tracking-widest ${isActive ? 'text-indigo-600' : 'text-gray-400 hover:text-gray-900'}`}>Studio</NavLink>
          </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-[100] bg-white/95 backdrop-blur-xl border-t border-gray-100 lg:hidden h-20 shadow-[0_-8px_30px_rgba(0,0,0,0.05)]">
        <div className="flex justify-around items-center h-full px-4 max-w-lg mx-auto">
          <NavLink to="/" className={({ isActive }: any) => `flex flex-col items-center gap-1.5 w-1/5 transition-all ${isActive ? 'text-indigo-600 scale-110' : 'text-gray-300'}`}>
          ...
          <NavLink to="/search" className={({ isActive }: any) => `flex flex-col items-center gap-1.5 w-1/5 transition-all ${isActive ? 'text-indigo-600 scale-110' : 'text-gray-300'}`}>
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
            <span className="text-[7px] uppercase font-black tracking-[0.2em]">Istraži</span>
          </NavLink>
          <NavLink to="/studio" className={({ isActive }: any) => `flex flex-col items-center gap-1.5 w-1/5 transition-all ${isActive ? 'text-indigo-600 scale-110' : 'text-gray-300'}`}>
          ...
          <NavLink to="/chat" className={({ isActive }: any) => `flex flex-col items-center gap-1.5 w-1/5 transition-all ${isActive ? 'text-indigo-600 scale-110' : 'text-gray-300'}`}>
